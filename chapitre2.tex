...
L’étude des besoins constitue une étape essentielle avant le développement d’un logiciel. Elle permet d’identifier les besoins fonctionnels et non fonctionnels afin de concevoir une application performante et fiable. Ce chapitre présente les principaux acteurs du système, une vue d’ensemble de l’application, l’architecture adoptée ainsi que le pilotage du projet avec Scrum.
...
Dans cette section, nous présentons les différents besoins fonctionnels de l’application selon chaque acteur. Ces besoins décrivent les fonctionnalités et les actions que chaque utilisateur peut réaliser afin d’assurer le bon fonctionnement de la plateforme et de répondre aux objectifs du projet. 
...
\begin{itemize}
    ...
    \item Gérer l’ensemble des éléments de la plateforme, notamment : les utilisateurs, les animaux à vendre, les animaux destinés à l’accouplement, les animaux proposés à l’adoption, les services animaliers, ainsi que les déclarations de perte et de trouvaille.
    \item Consulter son tableau de bord afin de suivre et analyser les statistiques des différents modules de la plateforme, notamment AniMatch, AniFound, AniMarket et AniCare.
\end{itemize} 
\newpage
...
\item \textbf{Ergonomie et expérience utilisateur} : Proposer une interface moderne, simple et intuitive, permettant aux utilisateurs de naviguer facilement entre les différentes fonctionnalités de la plateforme. L’application doit offrir une expérience fluide et cohérente grâce à une organisation claire des contenus, des interactions rapides et un design responsive adapté aux différents appareils (ordinateur, tablette et mobile). L’objectif est d’améliorer le confort d’utilisation et de faciliter l’accès aux services proposés.
\item \textbf{Scalabilité} : Assurer la capacité du système à gérer une augmentation progressive ou soudaine du nombre d’utilisateurs et des requêtes sans impact significatif sur les performances. Grâce à l’architecture microservices et à l’orchestration avec Kubernetes, chaque service peut être déployé et mis à l’échelle de manière indépendante selon la charge. Cette approche garantit une meilleure disponibilité, une répartition efficace des ressources et une évolution flexible de la plateforme.
...
\item \textbf{Résilience et tolérance aux pannes} : Assurer la continuité du service même en cas de défaillance d’un ou plusieurs composants du système. L’architecture doit permettre d’isoler les erreurs afin d’éviter leur propagation et de préserver le bon fonctionnement des services critiques. Grâce à l’utilisation de Kubernetes et de l’architecture microservices, le système peut redémarrer automatiquement les services défaillants et garantir une disponibilité continue de la plateforme.
...
\item \textbf{Observabilité} : Assurer une meilleure visibilité sur le fonctionnement du système grâce à la mise en place d’outils de monitoring et de visualisation tels que Prometheus et Grafana. Prometheus permet la collecte et le stockage des métriques en temps réel, tandis que Grafana offre des tableaux de bord pour surveiller l’état des services, analyser les performances et détecter rapidement les anomalies. En complément, les dashboards administratifs développés avec Recharts.js permettent de visualiser des statistiques métiers de manière claire et interactive pour faciliter la prise de décision.

\item \textbf{Portabilité} : Garantir la portabilité de l’application grâce à la conteneurisation avec Docker, permettant d’exécuter les services dans des environnements différents sans modification du code ni des dépendances. Cela assure une exécution homogène et fiable de la plateforme sur tout type d’infrastructure.
\end{itemize}  
\newpage
...
Cette figure \ref{fig:usecase_general} présente le diagramme global des cas d’utilisation de notre plateforme :
...
Dans cette section, nous présentons l’architecture du projet et justifions le choix adopté après comparaison des approches monolithique et microservices. 
\subsection{Architecture monolithique}

L’architecture monolithique regroupe toutes les fonctionnalités d’une application  dans une seule unité de déploiement, fortement couplée et exécutée dans un même processus\cite{ibm_monolithic_architecture}.
...
L’architecture microservices est une approche dans laquelle une application est décomposée en plusieurs services indépendants, chacun responsable d’une fonctionnalité métier spécifique. Chaque service est autonome et communique avec les autres via des API\cite{talend_microservices_guide}.
...
La figure \ref{fig:mono_micro} présente une comparaison entre les architectures monolithique et microservices :

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/micro vs mono.png}
    \caption{Comparaison architecture monolithique vs microservices}
    \label{fig:mono_micro}
\end{figure}

La table \ref{tab:mono_micro} illustre une comparaison détaillée entre les deux architectures selon plusieurs critères :

\begin{longtable}{|p{3.5cm}|p{6cm}|p{6cm}|}
\caption{Comparaison entre architecture monolithique et microservices\cite{atlassian_comparaison_monolithe_microservices}}
...
La figure \ref{fig:arch_anihub} illustre la structure générale de l’architecture microservices d’AniHub.
...
\begin{figure}[H]
    ...
    \caption{Logo Slack \cite{slack_logo}}
\end{figure}

Slack est une plateforme de communication collaborative permettant aux équipes de discuter, partager des fichiers et organiser le travail via des canaux dédiés. Elle facilite la coordination entre les membres en centralisant les échanges liés aux projets et en améliorant la fluidité de la communication au sein de l’équipe.
\vspace{0.3cm}
...
\begin{figure}[H]
    ...
    \caption{Logo draw.io \cite{drawio_logo}}
\end{figure}

draw.io est un outil en ligne permettant de créer facilement des diagrammes tels que les diagrammes UML, les organigrammes et les schémas d’architecture. Il est largement utilisé dans les projets logiciels pour modéliser les systèmes de manière claire et structurée. Grâce à son interface intuitive et ses nombreuses formes prédéfinies, il facilite la conception et la visualisation des différentes étapes de conception d’une application.
\vspace{0.3cm}
...
\begin{figure}[H]
    ...
    \caption{Logo Figma \cite{figma_logo}}
\end{figure}

Figma est un outil de conception d’interfaces utilisateur (UI/UX) collaboratif, utilisé pour créer des maquettes, prototypes et designs interactifs. Il permet aux équipes de travailler en temps réel sur les mêmes projets, ce qui facilite la conception, la validation et l’amélioration des interfaces. Grâce à ses fonctionnalités de prototypage, il aide à simuler le comportement des applications avant leur développement.
\vspace{0.3cm}
...
\begin{figure}[H]
    ...
    \caption{Logo Visual Studio Code \cite{vscode_logo}}
\end{figure}

Visual Studio Code est un éditeur de code source léger et puissant, supportant plusieurs langages de programmation et offrant des extensions pour améliorer la productivité des développeurs. Il propose des fonctionnalités comme l’autocomplétion, le débogage et l’intégration avec Git, ce qui facilite le développement et la gestion des projets logiciels.
\vspace{0.3cm}
...
\begin{minipage}{0.45\textwidth}
    ...
    \caption{Logo Git \cite{git_logo}}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=2cm]{images/github.png}
    \caption{Logo GitHub \cite{github_logo}}
\end{minipage}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Logo GitHub Actions \cite{githubactions_logo}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Logo TypeScript \cite{typescript_logo}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Logo Next.js \cite{nextjs_logo}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Logo Tailwind CSS \cite{tailwind_logo}}
\end{figure}
...
\begin{minipage}{0.45\textwidth}
    ...
    \caption{Logo Go \cite{golang_logo}}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
    \centering
    \includegraphics[width=3cm]{images/fiber.png}
    \caption{Logo Fiber \cite{fiber_logo}}
\end{minipage}
\end{figure}

Go est un langage de programmation compilé développé par Google, reconnu pour sa simplicité, sa performance et sa gestion efficace de la concurrence. Il est particulièrement adapté aux systèmes distribués et aux applications nécessitant de hautes performances. Fiber est un framework web rapide pour Go, inspiré d’Express.js, facilitant la création d’API performantes et légères, tout en simplifiant la gestion des requêtes HTTP et le développement de services backend.
\vspace{0.3cm}
...
\begin{figure}[H]
    ...
    \caption{Logo Neon PostgreSQL \cite{neon_logo}}
\end{figure}

Neon PostgreSQL est une plateforme de base de données serverless basée sur PostgreSQL, offrant scalabilité, performance et séparation du stockage et du calcul. Elle permet de gérer les bases de données de manière flexible, avec une meilleure optimisation des ressources et une adaptation automatique à la charge.
\vspace{0.3cm}
\newpage
...
\begin{figure}[H]
    ...
    \caption{Logo Postman \cite{postman_logo}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Logo RabbitMQ \cite{rabbitmq_logo}}
\end{figure}

RabbitMQ est un système de messagerie permettant la communication asynchrone entre services dans une architecture distribuée. Il assure le découplage des composants et améliore la scalabilité ainsi que la gestion des traitements en arrière-plan.
\vspace{0.3cm}
...
\begin{figure}[H]
    ...
    \caption{Logo Docker \cite{docker_logo}}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=2.5cm]{images/k8s.png}
    \caption{Logo Kubernetes \cite{kubernetes_logo}}
\end{figure} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=4cm]{images/prometheus.png}
    \caption{Logo Prometheus \cite{prometheus_logo}}
\end{figure}

Prometheus est un outil de monitoring et d’alerte open-source permettant de collecter et stocker des métriques en temps réel à partir des systèmes et services. Il est largement utilisé pour surveiller les architectures distribuées et microservices. 


\subsubsection{Grafana}
\begin{figure}[H]
    \centering
    \includegraphics[width=3cm]{images/grafana.png}
    \caption{Logo Grafana \cite{grafana_logo}}
\end{figure}

Grafana est une plateforme de visualisation et d’analyse de données permettant de créer des tableaux de bord interactifs à partir de différentes sources de données, notamment Prometheus, afin de surveiller l’état des systèmes en temps réel.
...
La figure \ref{fig:scrum_team} illustre les différentes parties prenantes de l’application :
...
Le backlog Scrum regroupe les user stories priorisées du projet. 
\newline 
 La table \ref{tab:product-backlog} présente le backlog produit de notre projet, organisé par sprint et classé selon la priorité des fonctionnalités. Chaque user story décrit une fonctionnalité du point de vue utilisateur avec un niveau de priorité : 
...
La priorisation des besoins est réalisée selon la méthode MoSCoW. La table \ref{tab:moscow} illustre les différentes catégories de cette méthode :
...
La table \ref{tab:sprint-plan} présente la planification des sprints ainsi que des releases du projet :
